...
The robot is the Unitree G1 from MuJoCo Menagerie with its Dex3 hands replaced by
BrainCo Revo2 hands (both downloaded by ``fetch_assets.py``); the combined model is
generated by :func:`build_robot_xml` and pulled in via ``<include>``.  Everything else
is generated from ``config.Layout`` so the controller can compute poses from the same
numbers.
"""
...
from fetch_assets import fetch, fetch_revo2
...
ROBOT_XML_NAME = "g1_revo2.xml"  # G1 body + Revo2 hands, generated next to the Menagerie files
...
# --------------------------------------------------------------------------------------
# robot: G1 body + Revo2 hands
# --------------------------------------------------------------------------------------
SIDES = ("left", "right")
REVO2_MESH_PREFIX = "revo2_"
# The Revo2 hand frame has +z along the fingers, +x out of the palm and the thumb on +y
# (right hand) / -y (left hand).  The G1 ``*_wrist_yaw_link`` frame has +x along the
# fingers, the palm towards the body's midline (+y right, -y left) and the thumb up (+z),
# so each mount is a pure permutation of axes.
REVO2_MOUNT_QUAT = {"right": "0.5 0.5 0.5 0.5", "left": "0.5 -0.5 0.5 -0.5"}
WRIST_FLANGE_X = 0.0415  # end face of the wrist_yaw_link mesh (where the Dex3 palm was bolted on)
ADAPTER_THICKNESS = 0.008  # wrist-to-hand adapter plate
REVO2_RGBA = "0.13 0.13 0.14 1"  # the Revo2 shell is dark grey
REVO2_PAD_RGBA = "0.30 0.30 0.33 1"  # fingertip touch pads


def _copy_revo2_meshes(revo2_dir: Path, g1_dir: Path) -> None:
    """Copy the Revo2 visual meshes next to the G1 meshes (prefixed) so one meshdir serves both
    and the low-poly decimation covers them too."""
    dst = g1_dir / "assets"
    for side in SIDES:
        for stl in (revo2_dir / "meshes" / "hands" / "visual" / side).glob("*.STL"):
            target = dst / f"{REVO2_MESH_PREFIX}{stl.name}"
            if not target.is_file():
                shutil.copyfile(stl, target)


def _revo2_hand_tree(revo2_dir: Path, side: str) -> Tuple[ET.Element, List[ET.Element]]:
    """The Revo2 body tree and its mesh assets, rewritten to live inside the G1 file.

    * collision geoms are dropped: the robot is kinematic and never collides
    * visual geoms keep ``class="visual"``, which now resolves to the G1's visual class
    * mesh files point at the prefixed copies in the G1 meshdir
    """
    root = ET.parse(revo2_dir / "mjcf" / f"revo2_{side}.xml").getroot()
    mesh_files = {m.get("name"): Path(m.get("file")).name for m in root.find("asset").findall("mesh")}
    hand = root.find("worldbody").find("body")
    used = []
    for body in hand.iter("body"):
        for geom in list(body.findall("geom")):
            if geom.get("class") != "visual":
                body.remove(geom)
                continue
            geom.set("rgba", REVO2_PAD_RGBA if "touch" in body.get("name", "") else REVO2_RGBA)
            used.append(geom.get("mesh"))
    meshes = [ET.Element("mesh", name=n, file=f"{REVO2_MESH_PREFIX}{mesh_files[n]}") for n in used]
    return hand, meshes


def build_robot_xml(g1_dir: Path, revo2_dir: Path, lowpoly: bool) -> Path:
    """Write ``g1_revo2.xml``: the Menagerie G1 with Dex3 hands swapped for Revo2 hands.

    Starting from ``g1_with_hands.xml`` (rather than the hand-less ``g1.xml``) keeps the
    wrist link geometry and the 29-DoF joint layout identical to the model the
    controller was tuned on; only what hangs off the wrist flange changes.
    """
    tree = ET.parse(g1_dir / "g1_with_hands.xml")
    root = tree.getroot()
    compiler = root.find("compiler")
    if lowpoly:
        compiler.set("meshdir", LOWPOLY_DIR_NAME)
    compiler.set("balanceinertia", "true")  # the Revo2 finger inertias need it
    for kf in root.findall("keyframe"):  # its qpos layout no longer matches
        root.remove(kf)
    asset = root.find("asset")
    for mesh in list(asset.findall("mesh")):
        if "_hand_" in (mesh.get("file") or ""):
            asset.remove(mesh)
    actuators = root.find("actuator")
    for act in list(actuators):
        if "_hand_" in act.get("joint", ""):
            actuators.remove(act)

    hand_pos = f"{WRIST_FLANGE_X + ADAPTER_THICKNESS:.4f} 0 0"
    adapter_pos = f"{WRIST_FLANGE_X + ADAPTER_THICKNESS / 2:.4f} 0 0"
    for side in SIDES:
        wrist = next(b for b in root.iter("body") if b.get("name") == f"{side}_wrist_yaw_link")
        for geom in list(wrist.findall("geom")):
            if "_hand_" in (geom.get("mesh") or ""):  # Dex3 palm
                wrist.remove(geom)
        for child in list(wrist.findall("body")):  # Dex3 finger chains
            wrist.remove(child)
        ET.SubElement(wrist, "geom", type="cylinder", size=f"0.028 {ADAPTER_THICKNESS / 2:.4f}", pos=adapter_pos,
                      quat="0.707107 0 0.707107 0", **{"class": "visual"}, rgba="0.2 0.2 0.21 1")
        hand, meshes = _revo2_hand_tree(revo2_dir, side)
        hand.set("pos", hand_pos)
        hand.set("quat", REVO2_MOUNT_QUAT[side])
        wrist.append(hand)
        asset.extend(meshes)

    ET.indent(tree, space="  ")
    out = g1_dir / ROBOT_XML_NAME
    tree.write(out, encoding="unicode")
    return out


def build_scene_xml(cfg: ScenarioConfig, g1_dir: Path, revo2_dir: Path, lowpoly: bool = True) -> str:
    L = cfg.layout
    gen = g1_dir / GEN_DIR_NAME
    make_textures(gen, cfg)
    _copy_revo2_meshes(revo2_dir, g1_dir)
    build_robot_xml(g1_dir, revo2_dir, lowpoly and _decimate_meshes(g1_dir))
    cans_xml, _ = _cans(L)
...
def write_scene(cfg: ScenarioConfig, lowpoly: bool = True) -> Path:
    g1_dir = fetch()
    revo2_dir = fetch_revo2()
    xml_path = g1_dir / SCENE_XML_NAME
    xml_path.write_text(build_scene_xml(cfg, g1_dir, revo2_dir, lowpoly=lowpoly))
    return xml_path
